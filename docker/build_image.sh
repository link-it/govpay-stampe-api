...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release binaria da utilizzare per il build (default: ${LATEST_GOVPAY_STAMPE_RELEASE})
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-stampe-api/releases/latest 2> /dev/null)"
LATEST_GOVPAY_STAMPE_RELEASE="${LATEST_LINK##*/}"
...
DOCKERBUILD_OPT=()
DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_stampe_fullversion=${VER:-${LATEST_GOVPAY_STAMPE_RELEASE}}")
[ -n "${TEMPLATE}" ] &&  cp -f "${TEMPLATE}" buildcontext/commons/
[ -n "${CUSTOM_GOVPAY_HOME}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_stampe_home=${CUSTOM_GOVPAY_HOME}")
[ -n "${CUSTOM_GOVPAY_LOG}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_stampe_log=${CUSTOM_GOVPAY_LOG}")
if [ -n "${CUSTOM_RUNTIME}" ]
then
...
if [ -n "${LOCALFILE}" ]
then
  DOCKERFILE="govpay-stampe/Dockerfile.daFile"
  cp -f "${LOCALFILE}" buildcontext/
else
  DOCKERFILE="govpay-stampe/Dockerfile.github"
fi
...
if [ -z "$TAG" ] 
then
    REPO=${REGISTRY_PREFIX}/govpay-stampe-api
  TAGNAME=${VER:-${LATEST_GOVPAY_STAMPE_RELEASE}}
  TAG="${REPO}:${TAGNAME}"  
fi
